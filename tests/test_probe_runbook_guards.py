"""Guards over the stratum-probe runbook, so its commands cannot drift from the code they run.

`docs/planning/p2-easier-stratum/probe-run/runbook.md` is the operator's command sheet for the
easier-stratum probe: the operator executes its arm command verbatim, and a later agent
executes its post-run commands verbatim (spec A4/A5, plan Phase 2). A command sheet that
disagrees with the code it runs fails at night, in a run nobody can undo, so the disagreements
are refused here first.

**This module is the A1 extension, not a parameterization.** The measured-arm guard
(`tests/test_runbook_guards.py`) is a frozen historical pin — its docstring narrates the
relative-workspace defect it caught, and its finding cites it as the correction that landed
test-first — so it is left byte-untouched and the parse helpers are imported from it **by
identity**: one parse implementation, shared, matching the repo's identity discipline
(`diffcheck` imports `classify_completion` by identity; the comparison re-derives the trigger
mapping by identity). The helpers are text-parameterized — every one takes the runbook text or
its parsed blocks as its argument — so only what a second sheet legitimately re-binds is
re-bound here: `RUNBOOK` and `STALE_WORKTREES`, plus the A2 candidate-resolution pin that is
this sheet's own.
...
anti-vacuity parse — this module pins the resolution rule (A2): every `--only` value the arm
command passes is a name the runbook's resolution block records, the excluded candidate is
named in the resolution block and in no `--only` value, and the block states the zero-ceiling
rule (a candidate with a measured zero retry-eligible ceiling is excluded by name,
`prd.md:93-103`).

**Watched failing first** (CONTRIBUTING.md:56-60): this module was written against a
deliberately wrong stub runbook — relative writable paths, no `--stratum`, a stale worktree
name — and refused it before the real sheet existed.
"""
...
def _arm_only_values(block: str) -> set[str]:
    """The values of every `--only` flag the arm command passes.

    `_arm_values` (imported by identity) maps flag → value, so a repeatable flag keeps only
    its last occurrence; the candidate restriction is exactly the repeatable one, and both
    values are the point of the A2 pin — hence this second walk over the same shell tokens.
    """
    tokens = shlex.split(block.partition(ARM_MODULE)[2], posix=True)
    return {
        tokens[index + 1]
        for index, token in enumerate(tokens)
        if token == "--only" and index + 1 < len(tokens)
    }


def _resolution_block(text: str) -> str:
    """The candidate-resolution section: the `## ` heading naming resolution and its body."""
...
def test_the_guard_imports_its_parse_helpers_by_identity() -> None:
    """The A1 discipline: one parse implementation, imported, never copied.

    Each helper is the measured-arm module's own object, so a fix that lands in the shared
    helpers is seen by both runbooks' guards — and a second, drifting copy cannot be
    introduced here.
    """
...
def test_the_parse_really_reads_the_runbooks_command() -> None:
    ...
    signature flags — the stratum document, the hardened switch, and the candidate
    restriction — must come out of the parse, proving the blocks being guarded are the
    runbook's own.
    """
    ...
    assert "--stratum" in arm, "the arm command no longer names the stratum document"
...
def test_every_flag_the_arm_command_passes_exists_in_the_parser() -> None:
    ...
    accepted by it, else the operator's night ends in a usage error. The parser side is
    pinned too: `--stratum`, `--retries` and `--only` must be present, proving this guard
    really saw the parser and not an empty one.
    """
    arm = _arm_block(_bash_blocks(_runbook()))
    assert arm
    parser_flags = {name for name in build_parser()._option_string_actions if name.startswith("--")}
    for flag in ("--stratum", "--retries", "--only"):
        assert flag in parser_flags, (
            f"the parser this guard checks does not accept `{flag}`, so either the import did "
            "not see the real `build_parser` or the probe's defining surface was removed"
        )
...
def test_the_runbook_names_exactly_one_worktree_everywhere() -> None:
    ...
    post-run target: the arm would write its `--out`, workspace and evidence into one
    checkout while the post-run commands read another, and the before/after comparison would
    compare two different trees' records.
    """
    text = _runbook()
    names = {
        name
...
def test_no_stale_worktree_name_survives_anywhere() -> None:
    """The three stale branch names are gone, everywhere, not just from the commands.
...
def test_the_resolution_block_names_and_excludes_the_candidates() -> None:
    ...
    The arm's `--only` values must equal the retained pair, and both must be names the
    resolution block records; the excluded candidate must be named in the resolution block
    and in no `--only` value; and the block must state the pre-committed exclusion rule (a
    measured zero retry-eligible ceiling excludes by name, `prd.md:93-103`) — so a sheet
    that starts scoring the excluded candidate, or drops a retained one, is refused here,
    before the night.
    """
    ...
    assert only_values, (
        "the arm command passes no --only flag, so the candidate set is unstated: a probe "
        "that sweeps every candidate spends the excluded one's share and this guard cannot "
        "tell"
    )
    assert only_values == set(RETAINED), (
        f"the arm command's --only values {sorted(only_values)} are not the retained pair "

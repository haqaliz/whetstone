...
#: The gitignored home of the arm's classifier counts — named as a pointer, never parsed.
LARGER_BASE_BREAKDOWN_HOME = "runs/larger-base-arm/"

#: A declared-not-yet-measured date for the larger-base document. An input, never the clock.
...
def _larger_base_document() -> LargerBaseReport:
    """The rendered larger-base document for the synthetic tallies, and its sidecars."""
    return build_larger_base_report(
        tallies=_larger_base_tallies(),
        contract=CONTRACT,
        candidate=LARGER_BASE_CANDIDATE,
        dev_subset=CONTRACT.dev_subset,
...
def test_the_larger_base_writer_writes_the_three_artifacts_and_nothing_else(
    tmp_path: Path,
) -> None:
    """report.md, report.json and cost.json — the same shape every home has.

    A reader learns one layout for all four report directories; a fourth file in any of
    them is a stray with a figure-shaped home. Nothing is written outside the destination.
    """
    into = tmp_path / "larger-base"
    written = write_larger_base_report(_larger_base_document(), into)
    ...
    assert sorted(path.relative_to(tmp_path).as_posix() for path in tmp_path.rglob("*")) == [
        "larger-base",
        "larger-base/cost.json",
        "larger-base/report.json",
        "larger-base/report.md",
    ], "WHY THIS IS A FAILURE: the writer touched a path outside the destination it was given"


def test_the_larger_base_document_is_a_pure_function_of_its_inputs() -> None:
    """The same inputs render byte-identical output, like every other report.

    The larger-base report is committed evidence once the arm runs; a reader who
    ...
    """
    first = _larger_base_document()
    second = _larger_base_document()
...
def test_the_larger_base_declaration_carries_no_count_and_no_contract() -> None:
    """The committed state of the home: declared, not yet measured, holding no figure.

    The arm has not run, so the committed artifacts cannot carry a figure — neither the
    arm's own (none exists) nor any existing home's (restating one is forbidden). The
    declaration says exactly that, names the four homes non-comparable, renders no
    ...
    assert "No count is measured here: the arm has not run." in document.markdown, (
        "WHY THIS IS A FAILURE: a directory whose arm has not run says nothing about "
        "that. A reader finding counts here later could not tell when they appeared"
    )
    assert "non-comparable" in document.markdown, (
        "WHY THIS IS A FAILURE: the declaration does not state that the four homes are "
    ...
    for home in (
        ...
        assert home in document.markdown, (
            "WHY THIS IS A FAILURE: the declaration does not name which directories' "
            "figures it does not restate"
        )
    ...
    assert payload["non_comparable"] is True, (
        "WHY THIS IS A FAILURE: the sidecar does not declare the arm's figures "
        "non-comparable — the machine-readable half of the changed-candidate-set ground"
    )
    assert payload["generation_contract"] is None, payload
    assert payload["per_candidate"] is None, payload


def test_the_larger_base_report_renders_every_contract_field_and_all_eight_rows() -> None:
    ...
    sentence beside the changed-candidate-set declaration, the denominator sentence,
    both pointers, and the token spend.
    """
    document = _larger_base_document()
    ...
    for row in (
        ...
        assert f"| `{row}` |" in markdown, (
            ...
            "table. The larger-base report carries the same eight rows as the other homes"
        )
    ...
    assert "61 private + 1 public = 62" in markdown, (
        "WHY THIS IS A FAILURE: the denominator sentence is missing. The arm's counts are "
        "stated over the harness's per-candidate denominator, which for a complete arm is "
        "the declared set less the dev-subset exclusions"
    )
    assert (
        ...
        "the changed-candidate-set declaration. The arm differs from all three existing "
        "homes in a pinned input — the model revision — and the report must say the "
        "figures are not comparable"
    )
    assert "new candidate" in markdown, (
        "WHY THIS IS A FAILURE: the changed-candidate-set ground is not stated. The whole "
        "argument the directory exists on is that the arm scores a new candidate"
    )
    assert LARGER_BASE_CANDIDATE in markdown, (
        "WHY THIS IS A FAILURE: the report names no candidate. The candidate the arm "
        "scores is the runbook's decision, and the pointer is how this document names it"
    )
    assert LARGER_BASE_BREAKDOWN_HOME in markdown, (
    ...
    assert "333.0" in markdown, (
        "WHY THIS IS A FAILURE: the token-spend disclosure is missing. The arm's spend "
        "must be stated, like every other home's"
    )
    assert payload["candidate"] == LARGER_BASE_CANDIDATE, payload
    assert payload["breakdown_home"] == LARGER_BASE_BREAKDOWN_HOME, payload
    assert payload["recorded_on"] == LARGER_BASE_RECORDED_ON, payload
...
def test_the_larger_base_report_refuses_figures_without_a_contract() -> None:
...
def test_the_larger_base_writer_reuses_the_shared_helpers_by_identity(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """The writer renders through the report module's own helpers — a copy would not feel a patch.

    The by-identity discipline asserted `is`: `build_larger_base_report` calls the
    report module's own `_row`, `_over`, `_contract_fields`, `_contract_block` and
    `_counts` by name, so a re-implementation (a copy) could never have been swapped
    for the module's own objects. The patch is the proof: replacing one helper's
    binding at the module changes the writer's output, which only holds when the
    writer resolves the helper from the module rather than carrying a copy of it.
    """
    helpers = {
        ...
        "_counts": lambda one: {"patched": True},
    }
    import whetstone.bakeoff.report as report_module
    ...
    for name, replacement in helpers.items():
        ...
        assert "patched" in document.markdown or "patched" in document.payload, (
            f"WHY THIS IS A FAILURE: patching the module's own {name} did not change the "
            "writer's output, so the writer carries a copy of the helper rather than "
            "resolving it by identity"
        )
        monkeypatch.undo()
...
def test_the_committed_larger_base_report_is_the_measured_render() -> None:
    """The home's committed state is the measured render, never a hand-typed document.

    Before the arm ran, the home's committed state was the writer's declaration — a
    direct writer call with no tallies, because the door refuses zero arms by design.
    The arm has now run and the door has rendered the measured series into the home
    (`--render-larger-base-report`, 2026-08-18). The durable invariant is the same in
    both states: the home holds the writer's output, never a hand-typed or hand-edited
    artifact. That is asserted here by the measured shape — the declaration's own
    sentence is gone, the contract's fields and the eight row labels are the writer's
    own rendering, and the sidecars carry the measured payload and cost record — while
    no specific count is pinned, because a figure about a model lives in this home and
    nowhere else.
    """
    home = REPO_ROOT / "reports" / "larger-base"
    markdown = (home / "report.md").read_text(encoding="utf-8")
    assert "No count is measured here: the arm has not run." not in markdown, (
        "WHY THIS IS A FAILURE: the committed report.md still holds the declaration's "
        "sentence. The arm has run and the door rendered the measured series; the home "
        "must hold that render, not the pre-run declaration"
    )
    for label in (
        "`solved`",
        "`coverage`",
        ...
        assert f"| {label} |" in markdown, (
            f"WHY THIS IS A FAILURE: the committed report.md lacks the writer's own "
            f"{label} row: the measured render is not what the writer would produce"
        )
    assert "retry budget 2" in markdown, (
        "WHY THIS IS A FAILURE: the committed report.md lacks the hardened contract's "
        "fields block, which the writer renders beside the counts"
    )
    payload = json.loads((home / "report.json").read_text(encoding="utf-8"))
    assert payload.get("schema") == "whetstone-larger-base-report/1", (
        "WHY THIS IS A FAILURE: the committed report.json does not carry the arm's "
        "schema"
    )
    assert payload.get("per_candidate"), (
        "WHY THIS IS A FAILURE: the committed report.json carries no measured per-"
        "candidate series: the arm has run, so the sidecar must hold its counts"
    )
    cost = json.loads((home / "cost.json").read_text(encoding="utf-8"))
    assert cost.get("generation_seconds") is not None, (
        "WHY THIS IS A FAILURE: the committed cost.json carries no generation cost: the "
        "arm's measured cost record is missing"
    )
    written = " ".join(
        (home / name).read_text(encoding="utf-8")
        for name in ("report.md", "report.json", "cost.json")
    )
    assert "No count is measured here: the arm has not run." not in written, (
        "WHY THIS IS A FAILURE: the committed home still states that no count is measured "
        "here. The arm has run and the door rendered the measured series; the declaration "
        "is the pre-run state and no longer the committed one"
    )
    assert re.search(r"\d+ of \d+", written), (
        "WHY THIS IS A FAILURE: the committed home renders no count. The arm has run and "
        "its series is published here — the measured render, never the declaration"
    )

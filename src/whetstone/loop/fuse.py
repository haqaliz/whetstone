...
from whetstone.loop.backend import MLX, UnknownRuntime, family
...
def fuser_for(checkpoint: Checkpoint) -> str:
    ...
    # `backend.family` owns the mapping, including the permanent tolerance of every `torch-*`
    # spelling. Resolving it a second time here is how the two callers come to disagree.
    try:
        return family(name)
    except UnknownRuntime as unknown:
        raise UnknownProvenance(
            f"{str(checkpoint.directory)!r} records backend {name!r}, which this repository has "
            f"no fuser for. Refused rather than approximated with the nearest one ({unknown})"
        ) from unknown
